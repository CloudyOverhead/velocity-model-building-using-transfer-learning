...
def plot_losses(logdir_1d, params_1d, logdir_2d, params_2d, plot=True):
    ...
    plt.savefig(join(FIGURES_DIR, "losses.png"), bbox_inches="tight", dpi=100)
    if plot:
        plt.gcf().set_dpi(200)
        plt.show()
    else:
...
def plot_real_models(dataset, pretrained, preds, plot=True):
    ...
    plt.savefig(
        join(FIGURES_DIR, "real_models.png"), bbox_inches="tight", dpi=100,
    )
    if plot:
        plt.gcf().set_dpi(200)
        plt.show()
...
def plot_real_stacks(dataset, inputs, preds, plot=True):
    ...
    plt.savefig(
        join(FIGURES_DIR, "real_stacks.png"), bbox_inches="tight", dpi=100,
    )
    if plot:
        plt.gcf().set_dpi(200)
        plt.show()
...
def plot_semblance(dataset, plot=True):
    ...
    plt.savefig(
        join(FIGURES_DIR, "semblance.png"), bbox_inches="tight", dpi=100,
    )
    if plot:
        plt.gcf().set_dpi(200)
        plt.show()
